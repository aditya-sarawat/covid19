...
let covid = {
	...
	setstateData: function ( data ) {
		...
		document.getElementById( "Vaccinated" ).innerText = vaccinated1.toString().replace( /\B(?=(\d{3})+(?!\d))/g, "," );
		...
		if ( value === "India" || value === "india" ) {
			...
			for ( let key in codes ) {
				if ( codes[key].toLowerCase() == value.toLowerCase() ) {
					...
					const { vaccinated1 } = data[key].total;
...
let addTableData = {
	createAndAdd: function ( data, date ) {
		...
		for ( let key in data ) {
			...
			vaccinated.innerText = data[key].total.vaccinated1.toString().replace( /\B(?=(\d{3})+(?!\d))/g, "," );
